...
import { Search, Filter, Download, UserPlus, MoreHorizontal, Edit, Trash2, Eye, Building2, CheckCircle2, XCircle } from "lucide-react"
...
const AdminOrganizationsPending = () => {
  ...
  const fetchOrganizations = async () => {
    setLoading(true)
    try {
      const response = await axiosInstance.get("/admin/get-pending-organization")
  ...
  const handleApproveOrganization = async (orgId) => {
    try {
      const response = await axiosInstance.put(`/admin/approve-organization-register`, {
  ...
  const handleRejectOrganization = async (orgId) => {
    try {
      const response = await axiosInstance.put(`/admin/reject-organization-register`, {
        organizationId: orgId,
  ...
  return (
    <div>
          ...
          {selectedOrg && (
            <div>
              ...
              <div className="mt-4">
                ...
                <p><strong>Số điện thoại:</strong> {selectedOrg.phone}</p>
                <p><strong>Trạng thái:</strong> {getStatusBadge(selectedOrg.organizationStatus)}</p>
                <p><strong>Trạng thái duyệt:</strong> {getStatusBadge(selectedOrg.adminStatus)}</p>
                <p><strong>Ngày tạo:</strong> {new Date(selectedOrg.createdAt).toLocaleDateString("vi-VN")}</p>
              </div>

              <div className="mt-4">
                <h5>Giấy phép hoạt động</h5>
                {selectedOrg.licenseDocuments && selectedOrg.licenseDocuments.length > 0 ? (
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {confirmAction === 'approve' ? 'Xác nhận duyệt tổ chức' : 'Xác nhận từ chối tổ chức'}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedOrg && (
            <div>
              ...
              <p className="mb-1">Ngày tạo: {new Date(selectedOrg.createdAt).toLocaleDateString("vi-VN")}</p>
