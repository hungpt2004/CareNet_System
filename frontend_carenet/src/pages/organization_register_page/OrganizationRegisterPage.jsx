...
const OrganizationRegisterPage = () => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      CustomFailedToast(
        error.response?.data?.message || "Đăng ký tổ chức thất bại!"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleLicenseSubmit = async (values) => {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        "/images/upload-organization-documents",
        formData,
        {
          headers: {
      ...
      CustomFailedToast(
        error.response?.data?.message || "Tải lên giấy phép thất bại!"
      );
    } finally {
      setLicenseLoading(false);
    }
            ...
            <Form form={form} layout="vertical" onFinish={handleSubmit}>
              <motion.div variants={formItemVariants}>
                ...
                >
                  <Input
                    prefix={
                      <Building2 size={16} className={styles.textPrimary} />
                    }
        ...
        <motion.div variants={cardVariants} initial="hidden" animate="visible">
          <Card className={`mb-6 ${styles.containerSecondary}`}>
            <motion.div className="mb-6" variants={formItemVariants}>
              ...
              <Text type="secondary">
                Vui lòng tải lên các giấy phép hoạt động của tổ chức để được xác
                minh
              </Text>
            </motion.div>
            <Form
              ...
              <motion.div variants={formItemVariants}>
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Vui lòng tải lên ít nhất 1 giấy tờ!",
                    },
                    {
                      validator: (_, value) => {
                        if (fileList && fileList.length > 0) {
                          const isLt5M = fileList.every(
                          ...
                          if (!isValidType) {
                            return Promise.reject(
                              "Chỉ chấp nhận file PDF, JPEG hoặc PNG!"
                            );
                          }
                        }
  ...
  return (
    ...
    >
      ...
      <motion.div className="mt-6 text-center mb-5" variants={formItemVariants}>
        <Text type="secondary">
          Bằng việc đăng ký, bạn đồng ý với các điều khoản và điều kiện của
          CareNet
        </Text>
      </motion.div>
    </motion.div>
