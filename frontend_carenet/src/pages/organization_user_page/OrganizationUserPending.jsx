...
const OrganizationUserPending = () => {
  ...
  const fetchEventRegistrations = async (eventId) => {
    try {
      ...
      const response = await axiosInstance.get(
        `/organization/get-request-pending/${eventId}`
      );
      if (response.data.status === "success") {
        console.log(
          "Fetched registrations:",
          response.data.eventRegistrationData
        );
  ...
  const handleApprove = async (requestId) => {
    try {
      ...
      const requestToApprove =
        selectedRequest || requests.find((r) => r._id === requestId);
      if (!requestToApprove) {
      ...
      const response = await axios.post(
        `http://localhost:5000/organization/approve-request/${requestId}`,
        {
  ...
  const handleReject = async (requestId) => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:5000/organization/reject-request/${requestId}`,
        {
  ...
  const handleSort = (key) => {
    ...
    const sorted = [...filteredRequests].sort((a, b) => {
      ...
      if (key === "registeredAt") {
        ...
        ["fullname", "email", "phone", "cccdNumber", "dob"].includes(key)
      ) {
              ...
              >
                ...
                <div>
                  ...
                  <div>
                    <Tag
                      color={
                        userData.status === "ready" ? "success" : "warning"
                      }
                    >
            ...
            >
              <Row gutter={[16, 24]}>
                <Col span={8}>
                  <Statistic
                    title="Điểm uy tín"
                    value={userData.reputationPoints}
              ...
              >
                ...
                <div>
                  ...
                  <div>
                    <Text strong>
                      {formatDate(selectedRequest.registeredAt)}
                    </Text>
                  </div>
                </div>
                <div>
    ...
    },
    {
      ...
      render: (user) => (
        <span
          style={{
            color:
              formatUserData(user).reputationPoints > 50
                ? "#52c41a"
                : "#faad14",
          }}
        >
            ...
            locale={{
              emptyText: (
                <div style={{ padding: 32, textAlign: "center" }}>
                  <div
                    style={{ fontSize: 72, color: "#d9d9d9", marginBottom: 16 }}
                  >
                    <UserOutlined />
                  </div>
