...
exports.uploadCCCD = [
  ...
  async (req, res) => {
    ...
    if (!req.files || req.files.length === 0) {
      return res.status(400).json({ error: true, message: "Không có ảnh CCCD để tải lên" });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: true, message: "Ảnh CCCD tải lên thất bại." });
    }
  }
];
// Remove a CCCD image by URL
exports.removeCCCD = asyncHandler(async (req, res) => {
  const user = req.user.user;
  const userId = user._id;
  ...
  if (!imageUrl) {
    return res
      .status(400)
      .json({ error: true, message: "Thiếu đường dẫn ảnh cần xóa." });
  }
