...
exports.getEventDetail = asyncHandler(async(req, res) => {
   ...
   try {
      ...
      const currentEvent = await Event.findOne({_id: id}).populate("organizationId","name description");
      // Get Feedback Event
      const feedbackArray = await Feedback.find({_id: {$in: currentEvent.feedbacks}}).populate("userId", "fullname")
...
exports.getMyEvents = asyncHandler(async(req, res) => {
   ...
   try {
      const myEvents = await HistoryEventModel.find({user: currentUser._id}).populate("event");
      return res.status(200).json({
         status: 'success',
